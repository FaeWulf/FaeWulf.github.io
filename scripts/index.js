...
Array.from(windowsTerminal).forEach(e => {
  if (window.innerWidth <= 800) {
    ...
    switch (e.id) {
      case "Visualizer":
        e.style.height = `${Math.floor(window.innerHeight * 0.5)}px`
        break
      case "Setting":
        e.lastElementChild.style.fontSize = "12px"
        e.style.height = `${Math.floor(window.innerHeight * 0.5)}px`
        break
      case "Note":
        e.style.height = `${Math.floor(window.innerHeight * 0.5)}px`
    ...
    e.style.left = "3px"
    e.style.width = `${Math.floor(window.innerWidth * 0.98)}px`
  }
  else {
    switch (e.id) {
      case "Terminal":
        e.style.height = `${Math.floor(window.innerHeight * 0.7)}px`
        e.style.width = `${Math.floor(window.innerWidth * 0.7)}px`
        break
      case "Visualizer":
        e.style.height = `${Math.floor(window.innerHeight * 0.4)}px`
        e.style.width = `${Math.floor(window.innerWidth * 0.2)}px`
        break
      case "Setting":
        e.style.height = `${Math.floor(window.innerHeight * 0.5)}px`
        e.style.width = `${Math.floor(window.innerWidth * 0.25)}px`
        break
      case "Note":
        e.style.height = `${Math.floor(window.innerHeight * 0.5)}px`
        e.style.width = `${Math.floor(window.innerWidth * 0.5)}px`
        break
      case "Matrix":
        e.style.height = `${Math.floor(window.innerHeight * 0.3)}px`
        e.style.width = `${Math.floor(window.innerWidth * 0.3)}px`
        break
    }
  }
});
