var strReg1 = /"(.*?)"/g,
    strReg2 = /'(.*?)'/g,
    numReg = /\b(\d+)/g,
    syntaxReg1 = /\b(new|if|else|do|while|switch|for|foreach|in|continue|break|return|typeof)(?=[^\w])/g,
    syntaxReg2 = /\b(document|window|Array|String|Object|Number|Function|function|var|const|let|\.length|\.\w+)(?=[^\w])/g,
    funcReg = /\b(function<\/span>)(\s+\w+)(\()(.*?)(?=[\)])(?=[^\w])/g,
    commentReg = /(\/\/.*)/g;

// Syntax Highlighting
function highlight(){
    $.each($('#codeIDE p'),function(){
        ...
        parsed = parsed.replace(syntaxReg1,'<span class="syntax">$1</span>');
        parsed = parsed.replace(syntaxReg2,'<span class="syntax2">$1</span>');
        parsed = parsed.replace(numReg,'<span class="number">$1</span>');
        parsed = parsed.replace(funcReg,'$1<span class="function">$2</span>$3<span class="function_args">$4</span>');
...
$(document).ready(function(){ 
    var el = document.getElementById('codeIDE');
    highlight(); 
    
    $('#codeIDE').keyup(function(e){
